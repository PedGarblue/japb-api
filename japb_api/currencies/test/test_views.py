...
class TestCurrencyViews(APITestCase):
    ...
    def test_api_get_currency_list_with_balance(self):
        ...
        account = Account.objects.create(name = 'Test Account', currency = main_currency, user = self.user)
        account_secondary = Account.objects.create(name = 'Test Account 2', currency = main_currency, user = self.user)
        foreign_account = Account.objects.create(name = 'Test Account 3', currency = foreign_currency, user = self.user)

        transactions_main = [
            Transaction(amount = MAIN_ACCOUNT_TRANSACTIONS_AMOUNTS[0], account = account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
            Transaction(amount = MAIN_ACCOUNT_TRANSACTIONS_AMOUNTS[1], account = account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        ]

        transactions_secondary = [
            Transaction(amount = SECONDARY_ACCOUNT_TRANSACTIONS_AMOUNTS[0], account = account_secondary, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
            Transaction(amount = SECONDARY_ACCOUNT_TRANSACTIONS_AMOUNTS[1], account = account_secondary, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        ]

        transactions_foreign = [
            Transaction(amount = 100, account = foreign_account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        ]
