...
def update_reports(account_pk):
    ...
    report = ReportAccount.objects.filter(user=account.user, account=account).order_by('-to_date').first()
    ...
    # get most recent report
    currency_report = ReportCurrency.objects.filter(user=account.user, currency=account.currency).order_by('-to_date').first()
