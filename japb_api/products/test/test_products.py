...
class TestProducts(APITestCase):
    def setUp(self):
        ...
        self.response_create = self.client.post(
            reverse("products-list"), self.data, format="json"
        )
    ...
    def test_api_create_product(self):
        ...
        )  # 1 from setUp and 1 from this test
    ...
    def test_api_create_product_can_create_with_category(self):
        ...
        self.assertEqual(
            Product.objects.get(pk=response.data["id"]).category.name, "Test Category"
        )
    ...
    def test_api_can_update_product(self):
        ...
        response = self.client.put(
            reverse("products-detail", kwargs={"pk": pk}), self.data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["name"], "Test Product")
        self.assertEqual(response.data["description"], "Test Product")
    ...
    def test_api_can_search_products_by_description(self):
        ...
        )  # Should find both products with "Test" in name or description
