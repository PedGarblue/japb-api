...
class TestAccountsViews(APITestCase):
    ...
    def test_api_get_accounts(self):
        nonuser = UserFactory()
        nonuserAccount = Account.objects.create(name='Non User Account', currency=self.currency, user=nonuser)
    ...
    def test_api_get_a_account_not_found(self):
        response = self.client.get(reverse('accounts-detail', kwargs={ 'pk': 999 }), format='json')
    ...
    def test_api_cant_access_other_user_account(self):
        nonuser = UserFactory()
        nonuserAccount = Account.objects.create(name='Non User Account', currency=self.currency, user=nonuser)
        response = self.client.get(reverse('accounts-detail', kwargs={ 'pk': nonuserAccount.id }), format='json')
    ...
    def test_api_cant_update_other_user_account(self):
        ...
        response = self.client.put(reverse('accounts-detail', kwargs={ 'pk': nonuserAccount.id }), data=new_data, format='json')
    ...
    def test_api_cant_delete_other_user_account(self):
        nonuser = UserFactory()
        nonuserAccount = Account.objects.create(name='Non User Account', currency=self.currency, user=nonuser)
        response = self.client.delete(reverse('accounts-detail', kwargs={ 'pk': nonuserAccount.id }), format='json')

        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_api_accounts_list_show_balances(self):
        account = Account.objects.get()
        TransactionFactory(amount=1064, description="transaction 1", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        TransactionFactory(amount=3046, description="transaction 2", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        TransactionFactory(amount=-4100, description="transaction 3", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC))
    ...
    def test_api_account_detail_shows_balance(self):
        account = Account.objects.get()
        TransactionFactory(amount=2010, description="transaction 1", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        TransactionFactory(amount=3050, description="transaction 2", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        TransactionFactory(amount=-5050, description="transaction 3", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC))
    ...
    def test_api_account_parses_decimals_correctly(self):
        ...
        TransactionFactory(amount=10000, description="transaction 1", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        TransactionFactory(amount=30050, description="transaction 2", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC)),
        TransactionFactory(amount=-15050, description="transaction 3", account=account, user = self.user, date=self.fake.date_time(tzinfo=pytz.UTC))
